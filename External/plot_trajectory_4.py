...
class WaypointAndRoverPlot(Node):
    def __init__(self):
        ...
        self.csv_filename = f"rover_log_{timestamp}.csv"
        self.csv_file = open(self.csv_filename, "w", newline="")
        self.csv_writer = csv.writer(self.csv_file)
        self.csv_writer.writerow(["time_sec", "x", "y", "z"])  # header
        ...
        # Reordered waypoints (your chosen index)
        index = [16,7,11,13,15,5,2,0,1,17,8,18,4,19,3,10,9,14,12,6]
        ord_waypoints = waypoints[index]

        self.wp_x = ord_waypoints[:, 0]
        self.wp_y = ord_waypoints[:, 1]

        # Generate spline path
        tck, u = splprep([self.wp_x, self.wp_y], s=0.5)
        ...
        self.ax.set_title("Reordered Waypoints (Spline) + Live Rover Path")
        ...
        self.ax.plot(self.spline_x, self.spline_y, 'g-', linewidth=2, label="Smooth Trajectory")
        self.ax.plot(self.wp_x, self.wp_y, 'bo', label="Reordered Waypoints")

        for i, (x, y) in enumerate(zip(self.wp_x, self.wp_y)):
            self.ax.text(x, y, f"{i+1}", color='blue', fontsize=9)
    ...
    def location_callback(self, msg):
        ...
        self.csv_writer.writerow([round(t, 3), msg.x, msg.y, msg.z])
        self.csv_file.flush()  # ensures real-time saving
